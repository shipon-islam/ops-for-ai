  ...
  return (
    <section className="bg-gray text-white">
      <div className="container">
        <div>
          {ourSteps.map((item) => (
            <div key={item.id} className="pt-8 pb-14 border-b border-gray-500">
              <p className="body-text font-semibold  flex gap-4">
                <span>0{item.id}</span> <span>{item.name}</span>
              </p>
              <div className="grid md:grid-cols-2 gap-y-12 gap-x-20 mt-8 md:mt-28">
                <div>
                  ...
                  <div className="flex items-center gap-4 mt-6 md:mt-8">
                    <Button  variant="outline" color="white"  className="capitalize">{item.button1.name}</Button>
                    <button className="flex items-center gap-2  px-4 py-2  rounded-md capitalize hover:text-blue transition-colors duration-300"><span>{item.button2.name}</span> <Icon icon="weui:arrow-outlined" width="12" height="24" /></button>
                  </div>
                </div>
                <div>
                  <Image
